...
1. Review the current `mppi_core` API surface: `GraspObservation`,
   `TactileState`, `MPPIOptimizer`, `DeltaQReferenceRolloutModel`,
   `GraspStabilityCost`, and `RobotCommand`.
2. Define the adapter boundary outside `mppi_core`: ROS joint/tactile messages
   become `GraspObservation`; `RobotCommand` becomes `ImpedanceCommands`.
3. Check whether contact kinematics, torque-residual force projection config,
   and NARI tactile conversion are available at the intended runtime call site.
4. Add or tighten deterministic tests for no-contact gating, invalid force-aware
   rollout, config parsing, and command dimensions before wiring into ROS.
5. Only after tests pass, choose the package/node that owns the ROS adapter and
   launch wiring.
...
- Default tactile rollout policy is `force_aware_required`; a kinematic fallback
  should remain an explicit ablation/debug path.
- `RobotCommand` is the final command packet, not the sampled MPPI action.
...
- Where should contact kinematics be constructed and cached for runtime rollout?
