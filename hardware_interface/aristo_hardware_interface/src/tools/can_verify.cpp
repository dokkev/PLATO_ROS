...
int main(int argc, char** argv) {
    ...
    while (std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count() < dur_sec) {
        const auto now = std::chrono::steady_clock::now();
        if (now >= next_tx) {
            const auto send_status = can.write(req);
            if (send_status != PCAN_ERROR_OK) {
                std::cerr << "CAN send failed with status 0x" << std::hex << send_status
                          << std::dec << ": " << pcan_interface::PCANInterface::format_error(send_status) << "\n";
                return 7;
            }
        ...
        TPCANMsg rx{};
        while (true) {
            const auto receive_status = can.read(rx);
            if (receive_status == PCAN_ERROR_QRCVEMPTY) {
                break;
            }
            if (receive_status != PCAN_ERROR_OK) {
                std::cerr << "CAN receive failed with status 0x" << std::hex << receive_status
                          << std::dec << ": " << pcan_interface::PCANInterface::format_error(receive_status) << "\n";
                return 8;
            }

            if (rx.LEN >= 7 && rx.DATA[0] == 0xF1) {
                float p=0, v=0, kp=0, kd=0, tq=0; bool in_oc=false, fault=false;
                decoder.get_states(rx, p, v, kp, kd, tq, in_oc, fault);
                std::cout << "RX 0xF1 from ID 0x" << std::hex << rx.ID << std::dec
