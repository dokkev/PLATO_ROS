...
def generate_launch_description():
    ...
    gazebo= ExecuteProcess(
        cmd=['gazebo', '-u', os.path.join(get_package_share_directory("plato_gazebo"), 'worlds', 'qarm_world_1.world'), "--verbose",
             "-s", "libgazebo_ros_factory.so",
             "-s", "libgazebo_ros_init.so", "--ros-args", "--params-file", os.path.join(get_package_share_directory("plato_gazebo"), 'config', 'params_init.yaml'), 
             ],
        output='screen'
    )
