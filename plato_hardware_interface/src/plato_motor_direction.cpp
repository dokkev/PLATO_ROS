...
PlatoMotorDirection::PlatoMotorDirection() 
    : motor_config_ {{
        {MOTOR_0_DIRECTION, MOTOR_0_ANGLE_OFFSET},
        {MOTOR_1_DIRECTION, MOTOR_1_ANGLE_OFFSET},
        {MOTOR_2_DIRECTION, MOTOR_2_ANGLE_OFFSET},
        {MOTOR_3_DIRECTION, MOTOR_3_ANGLE_OFFSET},
        {MOTOR_4_DIRECTION, MOTOR_4_ANGLE_OFFSET},
        {MOTOR_5_DIRECTION, MOTOR_5_ANGLE_OFFSET},
        {MOTOR_6_DIRECTION, MOTOR_6_ANGLE_OFFSET},
        {MOTOR_7_DIRECTION, MOTOR_7_ANGLE_OFFSET},
        {MOTOR_8_DIRECTION, MOTOR_8_ANGLE_OFFSET}
    }}
{
...
void PlatoMotorDirection::convert_joint_to_motor_effort(const std::vector<double>& joint_effort_commands, std::vector<double>& motor_effort_commands) {
    ...
    for (std::size_t i = 0; i < joint_effort_commands.size(); ++i) {
        ...
        motor_effort_commands[i] = motor_config_[i].is_cw ? joint_effort_commands[i] : -joint_effort_commands[i];
        motor_effort_commands[i] /= 0.06; // Convert torque to current, where torque constant is 0.06 Nm/A

    }
}

void PlatoMotorDirection::convert_joint_to_motor_position(const std::vector<double>& joint_position_commands, std::vector<double>& motor_position_commands) {
    // Ensure the output vector is correctly sized
    ...
    for (std::size_t i = 0; i < joint_position_commands.size(); ++i) {
        ...
        motor_position_commands[i] = motor_config_[i].is_cw ? joint_position_commands[i] : -joint_position_commands[i];
