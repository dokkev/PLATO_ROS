...
PlatoMotorDirection::PlatoMotorDirection()
    : motor_config_{{{MOTOR_0_DIRECTION, MOTOR_0_ANGLE_OFFSET},
...
void PlatoMotorDirection::convert_motor_to_joint_position(
  ...
  for (std::size_t i = 0; i < motor_position_states.size(); ++i) {
    ...
    // float alpha = 0.01;
    // double filtered_motor_position = alpha * motor_position_states[i] + (1 -
    // alpha) * motor_position_states_prev[i];

    // First, subtract the angle offset to normalize the position
    double normalized_position =
        motor_position_states[i] - motor_config_[i].angle_offset;
