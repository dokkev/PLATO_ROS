...
private:
    ...
    void publish_translation_twist(double x, double y, double z)
    {
        ...
        twist_msg->twist.linear.x = -y * translation_scale_;
        twist_msg->twist.linear.y = x * translation_scale_;
    ...
    void publish_rotation_twist(double roll, double pitch, double yaw)
    {
        auto twist_msg = std::make_unique<geometry_msgs::msg::TwistStamped>();
        twist_msg->header.stamp = this->now();
        twist_msg->header.frame_id = "ee"; // Adjust if necessary
        twist_msg->twist.angular.x = yaw * rotation_scale_;
        twist_msg->twist.angular.y = roll * rotation_scale_;
        twist_msg->twist.angular.z = pitch * rotation_scale_;
        twist_pub_->publish(std::move(twist_msg));
    }
